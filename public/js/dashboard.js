...
// NEW: Global plan state for new pages (Customer Insights & AI Automations)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ================= NEW: Load Real Plan & Update UI for New Pages =================
  async function loadUserPlan() {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        currentUserPlan = (data.plan || 'free').toLowerCase().trim();
        localStorage.setItem('currentPlan', currentUserPlan);
        console.log("Dashboard loaded real plan:", currentUserPlan);
        updatePlanUI();
      }
    } catch (err) {
      console.error("Failed to load real plan:", err);
    }
  }

  function updatePlanUI() {
    const isProOrAgency = currentUserPlan === 'pro' || currentUserPlan === 'agency';

    // Update plan display
    if (currentPlanSpan) {
      currentPlanSpan.textContent = currentUserPlan.toUpperCase();
      currentPlanSpan.style.color = isProOrAgency ? '#28a745' : '#dc3545';
    }

    // Update navigation links for new premium pages
    const insightsLink = document.querySelector('a[href="customer-insights.html"]');
    const automationsLink = document.querySelector('a[href="ai-automations.html"]');

    if (insightsLink) {
      if (isProOrAgency) {
        insightsLink.classList.remove('locked');
        insightsLink.innerHTML = 'Customer Insights <span style="color:#28a745;">✓</span>';
        insightsLink.onclick = null; // normal navigation
      } else {
        insightsLink.classList.add('locked');
        insightsLink.innerHTML = 'Customer Insights <span style="color:#dc3545;">Pro/Agency</span>';
        insightsLink.onclick = (e) => {
          e.preventDefault();
          alert('Customer Insights requires Pro or Agency plan. Go to pricing to upgrade.');
          // Optional: window.location.href = 'pricing.html';
        };
      }
    }
    ...
    if (automationsLink) {
      if (isProOrAgency) {
        automationsLink.classList.remove('locked');
        automationsLink.innerHTML = 'AI Automations <span style="color:#28a745;">✓</span>';
        automationsLink.onclick = null;
      } else {
        automationsLink.classList.add('locked');
        automationsLink.innerHTML = 'AI Automations <span style="color:#dc3545;">Pro/Agency</span>';
        automationsLink.onclick = (e) => {
          e.preventDefault();
          alert('AI Automations requires Pro or Agency plan. Go to pricing to upgrade.');
          // Optional: window.location.href = 'pricing.html';
        };
      }
    }
  ...
  async function loadDashboard() {
    try {
      ...
      localStorage.setItem("currentPlan", actualPlan); // ← THIS LINE ADDED: sync real plan so Customer Insights can read it instantly
      ...
      // NEW: Load real plan & update nav for new pages
      await loadUserPlan();
      ...
      window.dispatchEvent(new Event('storage'));
        ...
        const cached = localStorage.getItem("currentPlan") || "free";
  ...
  // NEW: Refresh plan when coming back (e.g. from pricing page)
