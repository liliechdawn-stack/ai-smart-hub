...
app.get("/api/automations/accounts", auth, (req, res) => {
    ...
    db.all(
        "SELECT id, platform, account_name, account_info, status, created_at, last_sync FROM connected_accounts WHERE user_id = ? AND status = 'active' ORDER BY created_at DESC",
        [userId],
        (err, rows) => {
            if (err) {
